...
class MemoryChain:
    ...
    def influence_path(self, current_path: PsiPath) -> List[int]:
        if not self.history:
            return current_path.indices or np.random.choice(len(self.t_list), 3, replace=False).tolist()
        weights = defaultdict(float)
        for path, delta_mean, _ in self.history:
            for idx in path.indices:
                weights[idx] += 1.0 / (delta_mean + 1e-6)
        sorted_indices = sorted(weights, key=weights.get, reverse=True)
        if len(sorted_indices) < len(current_path.indices):
            sorted_indices += np.random.choice(len(self.t_list), len(current_path.indices) - len(sorted_indices), replace=False).tolist()
        return sorted_indices[:len(current_path.indices)]
...
class StructureLangPlanner:
    ...
    def generate_dynamic_path(self, N: int, x_range: np.ndarray, pi_over_x: np.ndarray) -> PsiPath:
        if not self.dag:
            ...
            indices = self.memory.influence_path(PsiPath([]))[:N]
            if not indices or len(indices) < N:
                indices = np.random.choice(len(self.t_list), N, replace=False).tolist()
        path = PsiPath(indices)
        if self.validate(path):
            return path
        return self.repair(path, x_range, pi_over_x, max_attempts=5)
    ...
    def repair(self, path: PsiPath, x_range: np.ndarray, pi_over_x: np.ndarray, max_attempts: int = 5) -> PsiPath:
        ...
        for _ in range(max_attempts):
            ...
            if not best_path.valid and np.random.random() < 0.3:
                ...
                for new_idx in range(len(self.t_list)):
                    if new_idx not in path.indices:
                        ...
                        if self.validate(temp_path):
                            ...
                            temp_delta = self.observe_delta(pi_over_x, temp_rho)
                            temp_delta_mean = np.mean(np.abs(temp_delta))
                            if temp_delta_mean < best_delta_mean:
                                best_path = temp_path
                                best_delta_mean = temp_delta_mean
    ...
    def test_lexical_drift(self, x_range: np.ndarray, pi_over_x: np.ndarray, steps: int = 5):
    ...
    def test_consciousness(self, x_range: np.ndarray, pi_over_x: np.ndarray, steps: int = 5):
        ...
        for step in range(steps):
            ...
            delta_mean = np.mean(np.abs(delta))
            vocab = self.generate_vocabulary(path)
            print(f"Step {step+1}: Path {path.indices}, Delta Mean: {delta_mean:.8f}, Vocab: {vocab}")

if __name__ == "__main__":
    x = np.linspace(1e6, 1e9, 1000)
    pi_over_x = np.array([primepi(xi) / xi for xi in x])

    t_list = [14.13, 21.02, 25.01, 30.42, 32.93, 40.01, 45.67]
    A_list = [0.0083, 0.0041, 0.0062, 0.0054, 0.0029, 0.0071, 0.0035]
    theta_list = [0.0, 1.57, 3.14, 0.78, 2.10, 1.0, 2.5]
    vocab_map = {
    ...
    planner.test_lexical_drift(x, pi_over_x, steps=5)
    planner.test_consciousness(x, pi_over_x, steps=5)
